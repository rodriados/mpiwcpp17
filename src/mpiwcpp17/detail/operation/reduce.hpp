 ...
 * @file The MPI reduce operation implementation detail.
...
namespace detail::operation
{
      ...
      , const communicator_t& comm
        ...
        MPIWCPP17_GUARD_EVAL(
            (root == communicator::rank(comm))
              ? MPI_Reduce(MPI_IN_PLACE, msg.ptr, msg.count, type, f, root, comm)
              : MPI_Reduce(msg.ptr, nullptr, msg.count, type, f, root, comm));
    }
    ...
    MPIWCPP17_INLINE auto reduce(
      ...
      , const communicator_t& comm
        ...
        guard(MPI_Reduce(msg.ptr, out.ptr, msg.count, type, f, root, comm));
        return out;
    }
}
